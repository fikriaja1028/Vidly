...
class VideoRepositoryImpl @Inject constructor(
    ...
    private fun extractVideoStreams(streamInfo: StreamInfo, isLive: Boolean): List<StreamItem> {
        if (isLive) {
            streamInfo.hlsUrl?.let { url ->
                return listOf(StreamItem(url = url, quality = "Auto (Live)", format = "m3u8", isAdaptive = false))
            }
            return emptyList()
        }
        // Keep all streams – don't deduplicate by resolution string. Progressive (muxed)
        // and adaptive (video-only) variants at the same resolution (e.g. 720p) are
        // different itags and must both survive for correct quality selection & fallback.
        // We also preserve fps variants (e.g. 720p vs 720p60) which were previously
        // mis-sorted because `filter{isDigit}` turned "720p60" into 72060.
        val allStreams = mutableListOf<StreamItem>()
        streamInfo.videoStreams?.forEach {
            val res = it.getResolution() ?: "Unknown"
            allStreams.add(StreamItem(url = it.url ?: "", quality = res, format = it.format?.suffix ?: "mp4", isAdaptive = false))
        }
        streamInfo.videoOnlyStreams?.forEach {
            val res = it.getResolution() ?: "Unknown"
            allStreams.add(StreamItem(url = it.url ?: "", quality = res, format = it.format?.suffix ?: "webm", isAdaptive = true))
        }
        // Deduplicate only exact duplicates (same url + quality + format)
        val deduped = allStreams.distinctBy { "${it.quality}|${it.format}|${it.isAdaptive}|${it.url}" }
        return deduped.sortedByDescending { parseQualityToInt(it.quality) }
    }

    private fun parseQualityToInt(quality: String): Int {
        // Extract leading resolution number before 'p' (720p60 -> 720, 1080p -> 1080)
        // Avoid the old bug where "720p60".filter{isDigit} == "72060" ranked above 2160p.
        val match = Regex("""(\d+)\s*p""", RegexOption.IGNORE_CASE).find(quality)
        if (match != null) {
            val base = match.groupValues[1].toIntOrNull() ?: 0
            // Give slight boost to higher fps at same res so 720p60 > 720p, but never above next res
            val is60 = quality.contains("60", ignoreCase = true)
            return base * 10 + if (is60) 1 else 0
        }
    ...
    private fun extractSubtitles(streamInfo: StreamInfo): List<SubtitleItem> {
        return streamInfo.subtitles?.map {
            // FIX(subtitle mojibake): YouTube timedtext URLs may be delivered as
            // fmt=json3/srv3/ttml depending on NewPipe version. Force WebVTT for
            // ExoPlayer so the VTT decoder doesn't receive JSON/XML and show
            // mojibake (e.g. • -> • when UTF-8 bytes are mis-interpreted).
            val rawUrl = it.url ?: ""
            val vttUrl = rawUrl
                .replace("fmt=json3", "fmt=vtt")
                .replace("fmt=srv3", "fmt=vtt")
                .replace("fmt=ttml", "fmt=vtt")
                .let { u -> if (!u.contains("fmt=") && u.isNotBlank()) u + (if (u.contains("?")) "&" else "?") + "fmt=vtt" else u }
            SubtitleItem(url = vttUrl, languageTag = it.languageTag ?: "und", format = "vtt", isAutoGenerated = it.isAutoGenerated)
        ...
        return withContext(Dispatchers.IO) {
            ...
            for (url in candidates) {
                try {
                    ...
                    if (items.isNotEmpty()) {
                        VidlyLog.d("VideoRepository", "Shorts kiosk success via $url : ${items.size} items")
                        return@withContext PaginatedList(items, kiosk.nextPage)
                    }
                } catch (e: Exception) {
            ...
            try {
                ...
                if (topInterests.isNotEmpty()) {
                    val query = topInterests.joinToString(" ") { it.keyword } + " #shorts"
                    val youtubeService = ServiceList.YouTube
                    val extractor = youtubeService.getSearchExtractor(query, listOf("videos"), "relevance")
                    extractor.fetchPage()
                    val items = extractor.initialPage.items.filterIsInstance<StreamInfoItem>().map { mapToVideoItem(it) }
                    if (items.isNotEmpty()) {
                        VidlyLog.d("VideoRepository", "Shorts personalized search success: ${items.size} items for query: $query")
                        return@withContext PaginatedList(items.shuffled(), extractor.initialPage.nextPage)
                    }
                }
            } catch (e: Exception) {
                VidlyLog.d("VideoRepository", "Shorts personalized search failed: ${e.message}")
            }

            // 3. General #shorts search fallback
            try {
                val youtubeService = ServiceList.YouTube
                ...
                val items = extractor.initialPage.items.filterIsInstance<StreamInfoItem>().map { mapToVideoItem(it) }
                if (items.isNotEmpty()) {
                    VidlyLog.d("VideoRepository", "Shorts general search success: ${items.size} items")
                    return@withContext PaginatedList(items.shuffled(), extractor.initialPage.nextPage)
                }
            } catch (e: Exception) {
                VidlyLog.d("VideoRepository", "Shorts general search failed: ${e.message}")
            }

            // 4. Fallback: use trending but filter to short videos (duration <= 60s) for vertical experience
            try {
                val kiosk = KioskInfo.getInfo(ServiceList.YouTube, Constants.YouTube.TRENDING_URL)
                val all = kiosk.relatedItems.filterIsInstance<StreamInfoItem>().map { mapToVideoItem(it) }
                val shorts = all.filter { it.duration in 1..70 } // slightly wider margin
                val result = if (shorts.size >= 5) shorts else all.shuffled().take(20)
                VidlyLog.d("VideoRepository", "Shorts fallback trending: ${result.size} items (from ${all.size})")
                return@withContext PaginatedList(result, kiosk.nextPage)
            } catch (e: Exception) {
    ...
    override suspend fun fetchNextShortsPage(page: Page): PaginatedList<VideoItem> {
        ensureInit()
        return withContext(Dispatchers.IO) {
            // Try to continue previous Shorts kiosk; if original was trending, continue trending
            // We assume page was from getShortsVideos – try shorts first, then trending
            ...
            try {
                ...
                val items = next.items.filterIsInstance<StreamInfoItem>().map { mapToVideoItem(it) }
                val shorts = items.filter { it.duration in 1..65 }
                val result = if (shorts.isNotEmpty()) shorts else items
                return@withContext PaginatedList(result, if (next.hasNextPage()) next.nextPage else null)
            } catch (e: Exception) {
